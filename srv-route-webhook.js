...
async function handleUpdate(u) {
  ...
  const extra = PUBLIC_URL
    ? { reply_markup: { inline_keyboard: [[{ text: 'Open GPX Network', web_app: { url: PUBLIC_URL } }]] } }
    : {};
  ...
  if (s.welcome_photo_url) {
    await tgApi.sendPhoto(m.chat.id, s.welcome_photo_url, buildWelcomeHtml(s.welcome_text, s.welcome_emoji_ids, m.from), { ...extra, parse_mode: 'HTML' });
  } else {
    await tgApi.sendMessage(m.chat.id, buildWelcomeHtml(s.welcome_text, s.welcome_emoji_ids, m.from), { ...extra, parse_mode: 'HTML' });
  }
}
