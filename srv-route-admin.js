...
router.put('/settings',wrap(async(req,res)=>{const b=req.body||{},cur=await getSettings();const num=(v,d)=>{const n=Number(v);return Number.isFinite(n)?n:d};const referral_reward=num(b.referral_reward,cur.referral_reward);const gpx_per_001_usdt=Math.max(1,Math.floor(num(b.gpx_per_001_usdt,cur.gpx_per_001_usdt)));const min_conversion_gpx=Math.max(1,Math.floor(num(b.min_conversion_gpx,cur.min_conversion_gpx)));const withdrawals_per_day=Math.max(1,Math.floor(num(b.withdrawals_per_day,cur.withdrawals_per_day)));const min_withdraw=Math.max(0,num(b.min_withdraw,cur.min_withdraw));const max_withdraw=Math.max(0,num(b.max_withdraw,cur.max_withdraw));const withdrawal_fee=Math.max(0,num(b.withdrawal_fee,cur.withdrawal_fee));
  const ad_reward_gpx=Math.max(0,num(b.ad_reward_gpx,cur.ad_reward_gpx));
  const max_ads_per_day=Math.max(0,Math.floor(num(b.max_ads_per_day,cur.max_ads_per_day)));
  const required_ads_before_withdrawal=Math.max(0,Math.floor(num(b.required_ads_before_withdrawal,cur.required_ads_before_withdrawal)));
